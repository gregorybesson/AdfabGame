...
class Entry implements ServiceLocatorAwareInterface
{
    ...
    public function findLastEntriesBy($game, $user, $limitScale)
    {
        ...
        $query = $this->em->createQuery('SELECT COUNT(e.id) FROM AdfabGame\Entity\Entry e WHERE e.user = :user AND e.game = :game AND (e.bonus = 0 OR e.bonus IS NULL) AND e.created_at >= :date');
    ...
    public function findPlayersWithOneEntryBy($game)
    {
        ...
        $query = $this->em->createQuery(
            'SELECT e, u FROM AdfabGame\Entity\Entry e
            JOIN e.user u
            WHERE e.game = :game
    ...
    public function checkBonusEntry($game, $user)
    {
        ...
        $query = $this->em->createQuery('SELECT COUNT(e.id) FROM AdfabGame\Entity\Entry e WHERE e.user = :user AND e.game = :game AND (e.bonus = 0 OR e.bonus IS NULL)');
        $query->setParameter('user', $user);
        $query->setParameter('game', $game);
        $nbEntries = $query->getSingleScalarResult();

        $query = $this->em->createQuery('SELECT COUNT(e.id) FROM AdfabGame\Entity\Entry e WHERE e.user = :user AND e.game = :game AND e.bonus = 1');
        $query->setParameter('user', $user);
        $query->setParameter('game', $game);
    ...
    {
        if (null === $this->er) {
            $this->er = $this->em->getRepository('AdfabGame\Entity\Entry');
        }
